    ...
    public class PresentRepository : IPresentRepository
    {
        ...
        public async Task<(IEnumerable<Present> Items, int TotalCount)> GetAllPagedAsync(int pageNumber, int pageSize)
        {
            ...
            var items = await _context.Presents
                .Include(p => p.Category)
                .Include(p => p.Donor)
                .Include(p=>p.Purchases.Where(i=>!i.IsDraft))
        ...
        public async Task<int> getNumOfPurchasesForPresentAsync(int presentId)

        {
            int num = await _context.Purchases
                .CountAsync(p => p.PresentId == presentId && !p.IsDraft);
            return num;
        }
